...
We can now use the `parLapply` function just as we did with `lapply` but with the added `mc.cores` argument.

```{r eval=FALSE}
cluster <- parallel::makeCluster(nCores)

results <- parallel::parLapply(cl = cluster, tags, create_capture_hist, data = ratdat_med)
```
...
```{r eval=FALSE}
output <- foreach::foreach(i = 1:length(tags), .combine = rbind) %do% {
...
```{r eval=FALSE}
output <- foreach::foreach(i = 1:length(tags), .combine = rbind) %dopar% {
  create_capture_hist(tags[i], ratdat_med)
}
