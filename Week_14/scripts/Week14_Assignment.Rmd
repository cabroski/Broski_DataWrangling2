...
# 2a
seq_sqrt <- lapply(seq_list, sqrt)
seq_sqrt <- do.call(rbind, seq_sqrt)
...
seq_sqrt <- mclapply(seq_list, sqrt, mc.cores = nCores)

cluster <- parallel::makeCluster(nCores)

results <- parallel::parLapply(cl = cluster, seq_list, sqrt)
...
```{r}
source("../data_raw/dino_allometry_fxn.R")

dino_lengths <- read.csv("../data_raw/dinosaur_lengths.csv")
...
mass_output <- foreach::foreach(i = 1:nrow(dino_lengths), .combine = c) %do% {
...
mass_output <- foreach::foreach(i = 1:nrow(dino_lengths), .combine = c) %dopar% {
  mass_from_length(dino_lengths$lengths[i],
                  dino_lengths$species[i])
}
...
for (i in 1:nrow(forest)) {
  change_in_forest[i] <- forest$year_2021[i] - forest$year_1990[i]
}

head(change_in_forest)

# 4b
forest_output <- foreach::foreach(i = 1:nrow(forest), .combine = c) %do% {
   forest$year_2021[i] - forest$year_1990[i]
}

# 4c
doParallel::registerDoParallel(nCores)
forest_output <- foreach::foreach(i = 1:nrow(forest), .combine = c) %dopar% {
   forest$year_2021[i] - forest$year_1990[i]
}
doParallel::stopImplicitCluster()
