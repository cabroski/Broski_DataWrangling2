...
mass_output <- foreach::foreach(i = 1:nrow(dino_lengths), .combine = c) %do% {
  mass_from_length(lengths[i], species[i], dino_lengths)
}
...
change_in_forest <- vector(mode = "numeric", length = length(forest))
for (i in 1:length(forest)) {
  change_in_forest <- year_2021[i] - year_1990[i]
  print(change_in_forest[i])
}
