...
masses <- vector(mode = "numeric", length = length(lengths))

for (i in 1:length(lengths)) {
  mass <- mass_from_length(lengths[i], species[i])
  masses[i] <- mass
}
...
as <- c(10.95, 0.73, 214.44, 25.37)
bs <- c(2.64, 3.63, 1.46, 2.49)
...
# 2a
masses <- vector(mode = "numeric", length = length(lengths)) # change to masses?
  
for (i in 1:length(as)) {
  mass <- mass_from_length(as[i])
  masses[i] <- mass
}
masses
